...
echo "=== Gemini 3 Flash Preview Single-File Test on $(hostname) at $(date) ==="
...
echo ""
echo ">>> Step 2: Confirm gemini-3-flash-preview is accessible"
python -c "
from google import genai
import os
client = genai.Client(api_key=os.environ['GOOGLE_API_KEY'])
resp = client.models.generate_content(model='models/gemini-3-flash-preview', contents='Say hello in one sentence.')
print(f'gemini-3-flash-preview responded: {resp.text}')
"
if [ $? -ne 0 ]; then echo "FATAL: gemini-3-flash-preview not accessible — aborting"; exit 1; fi
...
echo ""
echo ">>> Calling Gemini 3 Flash Preview transcription..."
START_T=$(date +%s)
...
echo ""
echo "=== TEST PASSED — Gemini 3 Flash Preview is working ==="
