#!/usr/bin/env bash
# test_pipeline_14.sh — End-to-end pipeline on 10 test MP3 files
# Same as test_pipeline_13 but with expanded omegaconf safe globals fix:
# adds ContainerMetadata, ValueNode, BaseContainer for PyTorch 2.6 compatibility.
set -euo pipefail
...
echo "=== test_pipeline_14 on $(hostname) at $(date) ==="
cd "$REPO_DIR"
