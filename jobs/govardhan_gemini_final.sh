#!/bin/bash
echo "=== govardhan_gemini_final on $(hostname) at $(date) ==="
...
echo ">>> Testing API..."
python -c "import google.generativeai as genai; import os; genai.configure(api_key=os.environ['GOOGLE_API_KEY']); print(genai.GenerativeModel('gemini-2.0-flash').generate_content('Say hello').text)"
