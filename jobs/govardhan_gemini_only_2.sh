#!/usr/bin/env bash
# Job: Govardhan Gemini transcription v2 — with aiohttp==3.9.5 fix for Python 3.9
# Input:  /lab/kiran/govardhan/*.mp3
# Output: /lab/kiran/govardhan_transcripts/<stem>/transcript.json
# Fix:    aiohttp >=3.10 breaks TypedDict on Python 3.9 — downgrade to 3.9.5
set -euo pipefail
...
echo "============================================"
echo "  GOVARDHAN GEMINI TRANSCRIPTION v2"
echo "  $(hostname) — $(date)"
echo "============================================"

# Fix: downgrade aiohttp to be compatible with Python 3.9 TypedDict
