#!/usr/bin/env bash
# test_pipeline_12.sh — End-to-end pipeline on 10 test MP3 files
# Same as test_pipeline_11 but uses hardcoded vdabase Python path instead of
# conda activate (which silently fails in non-interactive SSH sessions on igpu15).
set -euo pipefail

# ---- Python from vdabase (bypass unreliable conda activate) ----
...
echo "=== test_pipeline_12 on $(hostname) at $(date) ==="
cd "$REPO_DIR"
...
for fname in "${TEST_FILES[@]}"; do
    ...
    $PYTHON - "$fuzz_dir/segments_pushed.json" "$final_transcript" <<'PYEOF'
import sys, json
...
def run_one(fname, gpu_id):
    ...
    result = subprocess.run(
        [os.environ.get("VDABASE_PYTHON", "python"), "04_diarization/split_audio_dia.py", fname],
